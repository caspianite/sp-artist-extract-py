...
class ArtistIndexConsumer:
    def __init__(self, database: database.DatabaseHandler, redis: database.RedisDatabaseHandler):
        self.database = database
        self.redis = redis
        self.connection = pika.BlockingConnection(pika.URLParameters(settings.rabbitmq_conn_string))

        self.user_client = UserClient(database, redis, self.connection)
        self.stop = False
        self.start_rabbitmq_consumer('sp_artist_pathfinder_index')
    ...
    def start_rabbitmq_consumer(self, queue_name: str):
        ...
        def callback(ch, method, properties, body):
            ...
            if self.user_client.process_artist_pathfinder(artist_key):
...
if __name__ == "__main__":
    ...
    consumer = ArtistIndexConsumer(db, redis)
